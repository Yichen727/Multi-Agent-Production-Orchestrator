...
| `run_search(query, project_id, user_id)` | ② Search | `search_agent` → `retrieval_service.hybrid_search` (event-aware clip recall; deterministic direct-retrieval fallback if the LLM is unavailable) |
...
The code path, the architecture diagram, and the thesis description therefore all match — there is no "documented supervisor, actually bypassed by the UI" discrepancy. The orchestrator also owns two structured-hand-off extractors that read a tool's **own message** rather than the model's prose, so the model reformatting its narration can never corrupt what the next stage receives: `_extract_plan` reads the Selection→Delivery timeline plan out of the planning tool message (`plan_timeline` or the event-based `plan_moment_timeline`), and `_extract_candidates` reads the Search stage's structured candidate list out of the `search_catalogue` / `list_all_shots` tool message (so the UI renders the real `hybrid_search` rows, not a re-typed list).
...
3. **Lexical overlap** — whole-word (never substring) matching against the clip's keywords/description; also the sole signal when embeddings are unavailable (no API key, or the demo seed).

The two signals combine with a **reinforcing (noisy-OR) rule**, so a strong hit in either layer drives relevance up rather than one diluting the other.

**Relevance is tiered by how trustworthy the evidence is.** A query is split into **core** terms (what the user actually asked for, translated to English) and **related** terms (the synonym expansion): full core coverage scores 0.9, synonym-only evidence caps at 0.6, and the two combine with `max` — expansion widens recall but can never certify a match. Matching is whole-word, so a search for "phone" no longer scores a clip tagged "microphone". Nothing content-derived may exceed **0.95**: the tags, the description and the embedding built from them all come from the vision model, so they are not independent witnesses and one mis-tag would inflate every layer at once — the percentage reports how strong the *catalogue's* evidence is, never ground truth. **Measured** constraints are the exception: a pure `orientation` / duration query reports a true **100%**, because ffprobe measured those. The Search stage runs by **invoking the Search Agent**: the LLM translates the query into a structured `search_catalogue` call (or `list_all_shots`), and the orchestrator reads the resulting candidate list back from that tool's own message (`_extract_candidates`) so the UI receives the exact `hybrid_search` rows rather than the model's prose. If no LLM is available, the agent errors, or it finishes a turn without searching, the stage **falls back to a deterministic direct-retrieval path** (query understanding + `hybrid_search`) so the UI always gets candidates — either way retrieval resolves to the same function. Query understanding (`hoist_orientation` + `expand_query_terms`) routes format words to filters and expands free text into English synonyms — a **constrained** expansion that returns the user's core terms and the synonyms separately, and never broadens a concrete entity into its category or its surroundings.
...
| [retrieval_service.py](app/services/retrieval_service.py) | Hybrid retrieval: `hybrid_search` (event-aware **clip** recall — the Search stage) and `search_events` (moment-level primitive — backs **Selection**), sharing one event-scoring path; + query understanding (`expand_query_terms`, `hoist_orientation`, `group_size`) + evidence-tiered relevance (whole-word core/related scoring, 0.95 cap on vision-inferred matches, 1.0 for ffprobe-measured filters). |
